...
| `prompts` | Prompt management: file-based `.md` templates under `resources/prompts/` (+ `guidelines.md`), `AvailablePrompts` enum registry, `Prompt`/`PromptCache` loading & `{{param}}` substitution, structured-output schemas (`StructuredOutputTemplate` + `structured_outputs/features/**`), wiring a prompt to its schema and operation key, streamed example lines (`» ` after a blank line) | [x] (10) |
