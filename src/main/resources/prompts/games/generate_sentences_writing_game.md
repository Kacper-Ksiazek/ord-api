...
Keep the topics casual and straightforward, allowing users to respond in just one or two sentences, rather than writing a full essay.

The game difficulty is set to **%%difficulty%%**, and the foreign language is **%%language%%** at **%%proficiency%%** level.

Generate **%%amountOfQuestions%%** word-topic pairs where:
- **word**: Use a word from the provided list
- **topic**: A clear and concise prompt in **%%generativeContentLanguage%%** language that encourages the user to write a sentence using the word. Do not include the word itself in the topic.
